...
from .forms import FrmCliente, FrmClienteUsr, FrmClienteContacto, FrmClienteFacturacion
...
def see(request, pk):
    ...
    if 'POST' == request.method:
        if 'add-home-add' == request.POST.get('action'):
            ...
            frm = FrmDireccion(instance=obj.direccion_oficina, data=request.POST)
            obj.direccion_oficina.updated_by = usuario
            dir = frm.save()
    ...
    return render(request, 'app/cliente/see.html', {
        ...
        'frm_direccion_oficina': FrmDireccion(instance=obj.direccion_oficina),
