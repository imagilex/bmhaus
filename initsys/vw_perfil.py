...
@valida_acceso(['group.agregar_perfiles_group'])
def new(request):
    usuario = Usr.objects.filter(id=request.user.pk)[0]
    if 'POST' == request.method:
...
def see(request, pk):
    ...
    usuario = Usr.objects.filter(id=request.user.pk)[0]
    toolbar = []
    if usuario.has_perm_or_has_perm_child('group.perfiles_group'):
        toolbar.append({
            'type': 'link',
            'view': 'perfil_index',
    ...
    if usuario.has_perm_or_has_perm_child(
        ...
        toolbar.append({
            ...
            'pk': pk})
    if usuario.has_perm_or_has_perm_child('group.eliminar_perfiles_group'):
        toolbar.append({
            'type': 'link_pk',
            'view': 'perfil_delete',
...
@valida_acceso(['group.actualizar_perfiles_group'])
...
@valida_acceso(['group.eliminar_perfiles_group'])
