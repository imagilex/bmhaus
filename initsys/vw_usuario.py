...
@valida_acceso(['usr.agregar_usuarios_user'])
def new(request):
    usuario = Usr.objects.filter(id=request.user.pk)[0]
    if 'POST' == request.method:
...
def see(request, pk):
    ...
    if usuario.has_perm_or_has_perm_child(
        ...
        toolbar.append({
            ...
            'pk': pk})
    if usuario.has_perm_or_has_perm_child('usr.eliminar_usuarios_user'):
        toolbar.append({
            'type': 'link_pk',
            'view': 'usuario_delete',
...
@valida_acceso(['usr.actualizar_usuarios_user'])
...
@valida_acceso(['usr.eliminar_usuarios_user'])
